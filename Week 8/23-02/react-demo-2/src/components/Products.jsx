...
function Products()
{
  ...
  const products = [
    ...
    return(
        <div className='text-center border-2 p-24 bg-blue-200 font-thin'>
      <h1 className='text-5xl'> Products Page </h1>
      <div className="grid grid-cols-5 bg-fuchsia-300">
      {
        // messages.map((msgObj,index)=><Test1 messages={msgObj} key = {index} />)
        products.map((prodObj,index)=><Product products={prodObj} key = {index} />)
      }
      </div>
    </div>
    )
